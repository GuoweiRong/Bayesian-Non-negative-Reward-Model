...
from utils import print_trainable_parameters, grm_compute_metrics , freeze_trainable_parameters
...
class ScriptArguments:
    ...
    dataset: Optional[str] = field(default='../dataset/Unified-Feedback/all')
    dataset_mode: Optional[str] = field(default='1k', metadata={"help": "use from '', '40k', and '400k' for the paper's experiments"},)
    # lora
    ...
    # eval
    per_device_eval_batch_size: Optional[int] = field(default=6)
    evaluation_strategy: Optional[str] = field(default="steps")
    eval_steps: Optional[int] = field(default=10)
    # model and loss
    base_model: Optional[str] =  field(default="../Model/gemma-2-2b-it")
    # log
    ...
    noise_ratio: Optional[float] = field(default=0.25)
...
# Load datasets
train_dataset, eval_dataset = load_train_eval_dataset(script_args.dataset, tokenizer, mode=script_args.dataset_mode, model_name='GRM',Label_noise=script_args.Label_noise ,noise_ratio=script_args.noise_ratio,size=1 if script_args.debug else None)
