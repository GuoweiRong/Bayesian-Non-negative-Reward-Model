...
from GRM_BNRM_trainer import GRMDataCollatorWithPadding, GRMRewardTrainer
from load_datasets import load_train_eval_dataset
from utils import print_trainable_parameters, grm_compute_metrics
from grm_utils import AutoModelForCausalLMWithValueHead,AutoModelForCausalLMWithBNValueHead 
...
class ScriptArguments:
    ...
    KL_ratio:Optional[float] = field(default=1e-5)
...
# Load datasets
train_dataset, eval_dataset = load_train_eval_dataset(script_args.dataset, tokenizer, mode=script_args.dataset_mode, model_name='GRM',Label_noise=script_args.Label_noise ,size=100 if script_args.debug else None)
