...
from BTBNRM_trainer import SimpleRewardTrainer, RewardDataCollatorWithPadding
from load_datasets import load_train_eval_dataset
from utils import print_trainable_parameters, BNBT_compute_metrics, freeze_trainable_parameters
import sys
# sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__),'..','..','rm_eval')))
# a=os.path.abspath(os.path.join(os.path.dirname(__file__),'..','rm_eval'))
# print(">>> Added to sys.path:", a)
from grm_utils import AutoModelForCausalLMWithBNValueHead
@dataclass
class ScriptArguments:
    ...
    num_train_epochs: Optional[float] = field(default=2, metadata={"help": "The number of training epochs for the reward model."})
    ...
    #flash_attention_2
    rm_only: Optional[bool] = field(
        default=False,
        metadata={'help': 'Only train the reward/value head and freeze the backbone and any LoRA modules.'}
    )
    # data flash_attention_2
    ...
    eval_steps: Optional[int] = field(default=40)
    # model and loss
    ...
    KL_ratio : Optional[float] = field(default=0.00001)
    # log
    ...
    save_steps: Optional[int] = field(default=40)
    debug: Optional[bool] = field(default=False, metadata={'help': 'if debug=True, only train with 100 samples'})
    Label_noise:Optional[bool] = field(default=False)
    deepspeed: Optional[str] = field(default=None, metadata={"help": "path to deepspeed config json/yaml"})
...
# Load datasets
train_dataset, eval_dataset = load_train_eval_dataset(script_args.dataset, tokenizer, mode=script_args.dataset_mode,Label_noise=script_args.Label_noise ,size=100 if script_args.debug else None)
...
model = AutoModelForCausalLMWithBNValueHead.from_pretrained(
...
if script_args.rm_only:
    # Only train v_head.
    freeze_trainable_parameters(model)

    for n, p in model.named_parameters():
        p.requires_grad = ("v_head" in n)
elif script_args.use_lora:
    # LoRA + v_head: PEFT freezes the backbone; keep v_head trainable.
...
trainer_params = {
    ...
    "compute_metrics": BNBT_compute_metrics,
    "data_collator": RewardDataCollatorWithPadding(tokenizer=tokenizer, max_length=script_args.max_length),
    'loss_type': script_args.loss_type,
    'weight_ratio': script_args.weight_ratio,
    'KL_ratio': script_args.KL_ratio,
...
# show_vhead_layers_and_dtypes(getattr(trainer.model, "module", trainer.model),
#                              "C after DS/Accelerate init")
