...
class BNValueHead(nn.Module):
    ...
    def __init__(self, config, **kwargs):
        ...
        if self.layer_type == 'linear':
            ...
            self.linear_l = self._build_mlp(hidden_size, num_neurons, num_neurons, num_layers)
            
            # linear_k: hidden_size -> 1
            self.linear_k = self._build_mlp(hidden_size, 1, num_neurons, num_layers)
            
            # linear_w: 1024 -> 1
            self.linear_w = nn.Linear(num_neurons, 1,bias=False)
            # self.w = nn.Parameter(torch.empty(num_neurons, 1))
            
            # linear_kw: 1 -> 1
            self.linear_kw = self._build_mlp(1, 1, num_neurons, num_layers)
            
            # linear: weibull_dim -> 1
            # self.value_bias = nn.Parameter(torch.zeros(1, dtype=self.linear_l[0].weight.dtype))
            self.value_bias = ScalarBias()
            # self.summary = nn.Sequential(
            #     self.linear_l,
            #     # self.linear_k,
            ...
            # self.summary = self._build_mlp(hidden_size, num_output, num_neurons, num_layers)

        # self.flatten = nn.Flatten()
        # import math
        # nn.init.kaiming_uniform_(self.w, a=math.sqrt(5), mode='fan_out')
        # self.weibull_dim = weibull_dim

    def _build_mlp(self, input_size, output_size, hidden_size, num_layers,use_bias: bool = True):
        
        module_list = []
        module_list.extend([nn.Linear(input_size, hidden_size,bias=use_bias), nn.ReLU()])
        # 中间层
        for _ in range(num_layers-1):
            module_list.extend([nn.Linear(hidden_size, hidden_size,bias=use_bias), nn.ReLU()])

        module_list.append(nn.Linear(hidden_size, output_size,bias=use_bias))
    ...
    def forward(self, hidden_states,attention_mask=None):
        ...
        # z_out = F.relu(self.linear_l(output))  # [batch_size,s, hidden_size]
        # z_out =  self.linear_l(output) 
        z_out = F.relu(self.linear_l(output)).clamp(min=1e-6, max=1e4)
        # k = self.linear_k(output)  # [batch_size, seq_len, 1]
        k   = F.softplus(self.linear_k(output)) .clamp(min=1e-6, max=1e4)
        k+=5
        weibull_lambda = z_out / torch.exp(torch.lgamma(1 + 1/k))
        # pre_out = z_out #   # [batch_size, s,hidden_size]
        ...
        if self.layer_type == 'linear':
            ...
            # last_layer = self.linear_w[-1]
            z_out_w = F.relu(self.linear_w.weight.transpose(1, 0))  #[num_neurons, 1]
            # z_out_w = F.relu(self.w)
            # z_out_w  = self.linear_w(output).squeeze(-1)
            # z_out_w = z_out_w.weight.transpose(1, 0)

        
       

        k_w = F.softplus(self.linear_kw(z_out_w)).clamp(min=1e-6, max=1e4)
        k_w+=5
        ...
        # hidden_last = pre_out[batch_indices, last_index]  # [batch_size, weibull_dim]
        ...
        gamma_shape = torch.tensor(1.0, dtype= torch.float32, requires_grad=False).cuda(non_blocking=True)
        gamma_scale = torch.tensor(1.0, dtype= torch.float32, requires_grad=False).cuda(non_blocking=True)
...
def load_model_withhead(model_name, peft_name, tokenizer, device, \
    ...
    if not len(peft_name):
        ...
        print('*************rm_eval********************************************************')
        print('*************Loading AutoModelForCausalLMWithValueHead*******************')
