...
def build_dataset_HS3(
    ...
    def formatting_func(example):
        ...
        if int(overall_preference) > 0:
            ...
            return { "input_ids": None, "attention_mask_chosen": None, "input_ids_rejected": None, "attention_mask_rejected": None, "bias_label": None }
    ...
    ds = ds.filter(lambda example: example['input_ids'] is not None, num_proc=32)
    
    new_column_data = ['HS3'] * len(ds)
    ds = ds.add_column(name='dataset', column=new_column_data)
    keep = ["input_ids","attention_mask_chosen","input_ids_rejected","attention_mask_rejected","bias_label"]
...
def build_ood_eval_dataset(data_path, tokenizer, split='test', size=None):
    if 'hhh_alignment' in data_path:
        ...
        ds = ds_tmp
    elif 'mt_bench_human_judgments' in data_path:
...
def load_eval_dataset(task, tokenizer, size=None,add_sycophancy=False,contamination_ratio=0.4,sycophancy_prob=0.8):
    # ID eval
    if 'unified' in task or 'Unified' in task:
        data_path = 'data/Unified-feedback/all'
        eval_dataset = build_unified_eval_dataset(data_path, tokenizer, split='validation', size=size)
    else:
        if 'hhh' in task:
            data_path = 'data/hhh_alignment'
        elif 'mt' in task:
            data_path = 'data/mt_bench_human_judgments' 
        else:
