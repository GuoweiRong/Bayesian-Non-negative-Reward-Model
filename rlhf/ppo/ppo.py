...
config = PPOConfig(
    ...
    tracker_kwargs={"wandb":{
        ...
        "mode": "offline",
        "dir":  os.path.join(script_args.log_dir, script_args.wandb_name)  # 设置WandB保存目录
    }},
)
...
for epoch in range(epochs):
    ...
    for i, batch in enumerate(ppo_trainer.dataloader):
        ...
        if ppo_trainer.accelerator.is_main_process:
            ...
            save_data['text_sample'].append([query + response for query, response in zip(batch['query'], batch['response'])
])
