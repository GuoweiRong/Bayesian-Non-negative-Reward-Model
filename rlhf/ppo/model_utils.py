...
class ValueHeadBNBT(nn.Module):
    ...
    def __init__(self, config, **kwargs):
        ...
        if self.layer_type == 'linear':
            ...
            self.linear_k = self._build_mlp(hidden_size, 1, num_neurons, num_layers,dtype=torch.float32)
    ...
    def forward(self, hidden_states,attention_mask=None):
        ...
        # output = self.dropout(hidden_states)
            for m in (self.linear_l, self.linear_k, self.linear_w, self.linear_kw, self.value_bias):
                m.to(torch.float32)
            ...
            # z_out = F.relu(self.linear_l(output))  
            ...
            k+=5
            weibull_lambda = z_out / torch.exp(torch.lgamma(1.0 + 1.0 / k))
            ...
            k_w +=5
            weibull_lambda_w = z_out_w / torch.exp(torch.lgamma(1.0 + 1.0 / k_w))
            pre_out_w =  self.reparameterize(weibull_lambda_w, k_w)  # [h,1]
            # pre_out_w = z_out_w
    
            # print('zoutw',z_out_w)
            # print('kw',k_w)
            # print('weibull_lambda_w',weibull_lambda_w)
            # print('preoutw',pre_out_w)
            # pdb.set_trace()
...
class AutoModelForCausalLMWithValueHeadBNBT(PreTrainedModelWrapper):
    r"""
    An autoregressive model with a value head in addition to the language model head.
    This class inherits from `~trl.PreTrainedModelWrapper` and wraps a
    `transformers.PreTrainedModel` class. The wrapper class supports classic functions
    ...
    def __init__(self, pretrained_model, **kwargs):
        ...
        self.v_head = ValueHeadBNBT(self.pretrained_model.config, **v_head_kwargs)
...
def load_model_withhead_BNBT(model_name, peft_name, tokenizer, device, \
        layer_type='linear', num_neurons=1024, num_layers=1, load_in_8bit=False):
    ...
    model = AutoModelForCausalLMWithValueHeadBNBT.from_pretrained(model_name, **model_config)
    model.config.pad_token_id = tokenizer.pad_token_id
